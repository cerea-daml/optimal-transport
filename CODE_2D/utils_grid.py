...
########################################
    ...
    def interpolationDefault_ResBoundary(self):
        centGrid = self.centGrid - self.stagGrid.interpolation()
        return CenteredGridResBound(self.M, self.N, self.P, centGrid,
                                    self.stagGrid.mx[0,:,:], self.stagGrid.mx[self.M+1,:,:],
                                    self.stagGrid.my[:,0,:], self.stagGrid.my[:,self.N+1,:],
                                    self.stagGrid.f[:,:,0],  self.stagGrid.f[1:self.M,1:self.N,self.P+1])
...
############################
# Class CenteredGridResBound
############################

class CenteredGridResBound:
    '''
    Class to deals with a centered grid and boundary conditions with a reservoir
    '''
...
#############
    ...
    def __repr__(self):
        return ( 'Centered grid and boundary conditions with reservoir with shape ' +
                 str(self.M) + ' x ' +
                 str(self.N) + ' x ' +
                 str(self.P) )
    
    def __delattr__(self, nom_attr):
        raise AttributeError('You can not delete any attribute from this class : CenteredGridResBound')

    def copy(self):
        return CenteredGridResBound( self.M, self.N, self.P, self.centGrid.copy(),
                                     self.bx0.copy(), self.bx1.copy(),
                                     self.by0.copy(), self.by1.copy(),
                                     self.bt0.copy(), self.bt1.copy() )
    ...
    def random(M, N, P):
        ...
        bt0 = np.random.rand(M+1,N+1)
        bt1 = np.random.rand(M-1,N-1)
        return CenteredGridResBound( M, N, P, centGrid,
                                     bx0, bx1,
                                     by0, by1,
                                     bt0, bt1 )
    random = staticmethod(random)
...
########################################
    ...
    def T_interpolationDefault_ResBoundary(self):
        mxu = np.zeros(shape=(self.M+2,self.N+1,self.P+1))
        myu = np.zeros(shape=(self.M+1,self.N+2,self.P+1))
        fu  = np.zeros(shape=(self.M+1,self.N+1,self.P+2))
        ...
        fu[:,:,1:self.P+2]             -= 0.5*self.centGrid.f[:,:,:]
        fu[:,:,0]                      += self.bt0[:,:]
        fu[1:self.M,1:self.N,self.P+1] += self.bt1[:,:]
...
##########################
    ...
    def __add__(self, other):
        if isinstance(other,CenteredGridResBound):
            return CenteredGridResBound(self.M, self.N, self.P, self.centGrid + other.centGrid,
                                        self.bx0 + other.bx0, self.bx1 + other.bx1,
                                        self.by0 + other.by0, self.by1 + other.by1,
                                        self.bt0 + other.bt0, self.bt1 + other.bt1)
        else:
            return CenteredGridResBound(self.M, self.N, self.P, self.centGrid + other,
                                        self.bx0 + other, self.bx1 + other,
                                        self.by0 + other, self.by1 + other,
                                        self.bt0 + other, self.bt1 + other)
    ...
    def __radd__(self, other):
        return CenteredGridResBound(self.M, self.N, self.P, other + self.centGrid,
                                    other + self.bx0, other + self.bx1,
                                    other + self.by0, other + self.by1,
                                    other + self.bt0, other + self.bt1)
    ...
    def __neg__(self):
        return CenteredGridResBound(self.M, self.N, self.P, - self.centGrid,
                                    - self.bx0, - self.bx1,
                                    - self.by0, - self.by1,
                                    - self.bt0, - self.bt1)
    ...
    def __abs__(self):
        return CenteredGridResBound(self.M, self.N, self.P, abs(self.centGrid),
                                    np.abs(self.bx0), np.abs(self.bx1),
                                    np.abs(self.by0), np.abs(self.by1),
                                    np.abs(self.bt0), np.abs(self.bt1))
