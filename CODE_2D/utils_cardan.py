...
def solutionPQ(p,q):
    d = delta(p,q)
    rac_d = np.sqrt(np.abs(d))
    u = 0.5*(-27.*q+3.*np.sqrt(3.)*rac_d)
    v = 0.5*(-27.*q-3.*np.sqrt(3.)*rac_d)
    rac_3_u = -np.power(np.abs(u),1./3.)*(u<0) + np.power(np.abs(u),1./3.)*(1.-(u<0))
    rac_3_v = -np.power(np.abs(v),1./3.)*(v<0) + np.power(np.abs(v),1./3.)*(1.-(v<0))
    ...
    return root0*(d>0)+root1*(1.-(d>0)),root0*(d>0)+root2*(1.-(d>0)),root0*(d>0)+root3*(1.-(d>0))
